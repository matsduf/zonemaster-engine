...
@res = Zonemaster::Engine->test_method( 'Connectivity', 'connectivity01', Zonemaster->zone( q{afnic.fr} ) );
...
@res = Zonemaster::Engine->test_method( 'Connectivity', 'connectivity02', Zonemaster->zone( q{afnic.fr} ) );
ok( ( any { $_->tag eq 'NO_NETWORK' } @res ), 'IPv6 and IPv4 disabled' );
ok( ( none { $_->tag eq 'IPV6_DISABLED' } @res ), 'No network' );
ok( ( none { $_->tag eq 'IPV4_DISABLED' } @res ), 'No network' );
