...
@res = Zonemaster::Engine->test_method( 'Delegation', 'delegation04', Zonemaster->zone( q{iis.se} ) );
...
@res = Zonemaster::Engine->test_method( 'Delegation', 'delegation06', Zonemaster->zone( q{iis.se} ) );
ok( ( any { $_->tag eq 'NO_NETWORK' } @res ), 'IPv6 and IPv4 disabled' );
ok( ( none { $_->tag eq 'IPV6_DISABLED' } @res ), 'No network' );
ok( ( none { $_->tag eq 'IPV4_DISABLED' } @res ), 'No network' );
